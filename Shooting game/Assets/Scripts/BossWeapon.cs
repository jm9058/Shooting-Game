...
public class BossWeapon : MonoBehaviour
{
    ...
    private IEnumerator CircleFire()
    {
        ...
        while ( true )
        {
            for ( int i = 0; i < count; i++ )
            {
                ...
                // 발사체 이동 방향 (벡터)
                float x = Mathf.Cos(angle * Mathf.PI / 180.0f); // Cos(각도), 라디안 단위 각도 표현을 위해 PI / 180을 곱함
                float y = Mathf.Sin(angle * Mathf.PI / 180.0f); // Sin(각도), 라디안 단위 각도 표현을 위해 PI / 180을 곱함
                // 발사체 이동 방향 설정
    ...
    {
        Vector3 targetPosiotion = Vector3.zero; // 목표 위치 (중앙)
        ...
        while (true)
        {
            ...
            GameObject clone = Instantiate(projectilePrefab, transform.position, Quaternion.identity);
            // 발사체 이동 방향
            Vector3 direction = (targetPosiotion - clone.transform.position).normalized;
            // 발사체 이동 방향 설정
